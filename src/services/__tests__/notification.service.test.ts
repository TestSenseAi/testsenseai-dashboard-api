...
describe('NotificationService', () => {
  ...
  describe('notifyAnalysisComplete', () => {
    ...
    it('should handle notification errors', async () => {
      ...
      await expect(
        service.notifyAnalysisComplete('org-123', 'analysis-123', mockResult)
      ).rejects.toThrow(InternalError);
    });

    it('should handle connection retrieval errors', async () => {
      jest
        .spyOn(service as any, 'getOrganizationConnections')
        .mockRejectedValueOnce(new Error('Failed to get connections'));
  ...
  describe('notifyAnalysisFailed', () => {
    ...
    it('should handle notification errors', async () => {
      ...
      await expect(
        service.notifyAnalysisFailed('org-123', 'analysis-123', 'Analysis failed')
      ).rejects.toThrow(InternalError);
    });
