...
jest.mock('@nitric/sdk', () => ({
    kv: jest.fn().mockReturnValue({
        ...
        get: mockGet,
        set: mockSet.mockImplementation((_analysisId: string, _data: any) => Promise.resolve()),
    }),
}));
...
const mockNotificationService = {
    socket: { send: jest.fn() },
    notifyAnalysisComplete: jest.fn<Promise<void>, [string, string, unknown]>(),
    notifyAnalysisFailed: jest.fn<Promise<void>, [string, string, string]>(),
...
describe('AnalysisService', () => {
    ...
    beforeEach(() => {
        ...
        mockList.mockReset();
        mockList.mockImplementation((orgId: string, _options: any) => {
            const analyses = [
                {
                    id: 'analysis-1',
                    orgId: 'org-123',
            ...
            return {
                items: analyses.filter(a => a.orgId === orgId),
                nextCursor: '2024-02-20T10:00:00Z',
            };
        });

        service = new AnalysisService(
            { analyzeTest: mockAnalyzeTest, getHealth: mockGetHealth } as any,
            mockNotificationService as any
        );

        // Mock processAnalysis for successful case
        jest.spyOn(service as any, 'processAnalysis').mockImplementation(function (this: any, _analysis: any) {
    ...
    describe('createAnalysis', () => {
        ...
        it('should process analysis successfully', async () => {
            const orgId = 'org-123';
            jest.spyOn(service as any, 'processAnalysis').mockImplementationOnce(async function (
                this: any,
                analysis: any
            ) {
                await service['updateAnalysisResult'](analysis.id, {
                    confidence: 0.85,
                    summary: 'Test completed',
                    recommendations: [],
                });
        ...
        it('should handle processing errors', async () => {
            ...
            jest.spyOn(service as any, 'processAnalysis').mockImplementationOnce(async function (
                ...
                await service['updateAnalysisError'](analysis.id, {
        ...
        it('should create and process analysis successfully', async () => {
            const mockRequest = {
                ...
                options: {
                    ...
                    notifyOnCompletion: true,
                    analysisDepth: 'detailed' as 'detailed' | 'basic' | 'comprehensive',
            ...
            // Mock processAnalysis to resolve immediately
            jest.spyOn(service as any, 'processAnalysis').mockImplementation(function (this: any, ...args: any[]) {
                const analysis = args[0] as {
                    id: string;
                    orgId: string;
                    context: { projectId: string; testId: string; parameters: any; metadata: any };
                };
                return (async () => {
                    await mockAnalyzeTest({
                    ...
                    await service['updateAnalysisResult'](analysis.id, mockResponse);
                    await mockNotificationService.notifyAnalysisComplete(analysis.orgId, analysis.id, {
            ...
            expect(lastSetCall[0]).toBe(result.id);
        ...
        it('should handle processing error', async () => {
            ...
            // Mock processAnalysis to reject immediately
            jest.spyOn(service as any, 'processAnalysis').mockImplementation(function (this: any, ...args: any[]) {
                const analysis = args[0] as {
                    id: string;
                    orgId: string;
                ...
                return (async () => {
                    ...
                    await mockNotificationService.notifyAnalysisFailed(analysis.orgId, analysis.id, error.message);
                    throw error;
                })();
            });
